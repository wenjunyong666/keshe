...
/* 新增：默认地址改为可运行时修改，兼容菜单里的“重置/配对”功能 */
...
{
  /* 对应要求8“无线配对”：
   * 菜单选择“重置”或“配对”后，通过本函数把新的频道和地址写到底层 NRF 驱动。
   */
  /* 新增：由上层菜单写入新的频道/地址，底层发射接收模式自动复用 */
  uint8_t i;
...
void RX2TX(void)
{
  ...
  NRF24L01_Write_Reg(NRF24L01_WRITE_REG+RF_SETUP,0x0b);
